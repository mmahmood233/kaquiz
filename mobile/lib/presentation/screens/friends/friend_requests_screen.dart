...
// It shows requests loaded from the backend and lets the user accept/decline.
...
// Incoming requests can be accepted/declined; outgoing requests are read-only.
...
class _FriendRequestsScreenState extends State<FriendRequestsScreen>
  ...
  // Controls the Received and Sent tab views.
  late TabController _tabController;

  // Tracks requests currently being processed so buttons cannot be tapped twice.
  ...
  void initState() {
    ...
    // Load pending invites from the backend after the screen first renders.
  ...
  @override
  void dispose() {
    // Dispose tab controller because this screen created it.
    _tabController.dispose();
    super.dispose();
  }

  // Accepts or declines one incoming request.
  // FriendViewModel calls /api/invites/{sender_id}/accept or /decline.
  Future<void> _handleResponse(
    ...
    // Pass senderUserId because the backend route identifies the sender.
  ...
  Widget build(BuildContext context) {
    // Top tabs split backend invites into Received and Sent lists.
    return Column(
      children: [
        Container(
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [_buildIncomingTab(), _buildOutgoingTab()],
          ),
        ),
      ],
    );
  ...
  // Received requests tab: requests other users sent to this account.
      ...
      builder: (context, vm, _) {
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (ctx, i) => _incomingCard(ctx, vm.pendingRequests[i]),
          ),
        );
      },
    );
  ...
  // Sent requests tab: requests this account sent to other users.
  ...
  // Card for one received request with Accept and Decline buttons.
  ...
  // Card for one outgoing request waiting for the other user.
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Icon(Icons.schedule_rounded, size: 12, color: AppTheme.info),
  ...
  // Shared empty-state UI for Received and Sent tabs.
  Widget _emptyState({
